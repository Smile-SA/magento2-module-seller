...
class Eav extends AbstractModifier
{
    ...
    {
        if (!$this->locator->getSeller()->getId() && $this->dataPersistor->get(SellerInterface::ENTITY)) {
            return $this->resolvePersistentData($data);
        }

        $sellerId = $this->locator->getSeller()->getId();
        ...
        foreach (array_keys($this->getGroups()) as $groupCode) {
            ...
            foreach ($attributes as $attribute) {
                if (null !== ($attributeValue = $this->setupAttributeData($attribute))) {
                    ...
                    $data[$sellerId][self::DATA_SOURCE_DEFAULT][$attribute->getAttributeCode()] = $attributeValue;
                }
            }
        }
    ...
    {
        $persistentData = (array)$this->dataPersistor->get(SellerInterface::ENTITY);
        $this->dataPersistor->clear(SellerInterface::ENTITY);
        $sellerId = $this->locator->getSeller()->getId();

        if (empty($data[$sellerId][self::DATA_SOURCE_DEFAULT])) {
            $data[$sellerId][self::DATA_SOURCE_DEFAULT] = [];
        }

        $data[$sellerId] = array_replace_recursive(
            $data[$sellerId][self::DATA_SOURCE_DEFAULT],
            $persistentData
        );
    ...
    {
        return 19 ; //$this->locator->getSeller()->getAttributeSetId();
    }
    ...
    /**
     * Loading seller attributes from group
     *
    ...
    private function loadAttributes(AttributeGroupInterface $group)
    {
        ...
        //$sellerType = $this->getProductType();
        foreach ($groupAttributes as $attribute) {
            /*$applyTo = $attribute->getApplyTo();
            $isRelated = !$applyTo || in_array($sellerType, $applyTo);
            if ($isRelated) {*/
    ...
    private function getValue(ProductAttributeInterface $attribute)
    {
        ...
        return $seller->getData($attribute->getAttributeCode());
    }
